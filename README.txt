...
- 场次成片支持“片名图片”：导入透明 PNG（也可 JPG/WebP，5 MB 内），在本场最后5秒居中淡入。原图本地叠加，不交给视频模型重画；更换片名只重新合成，不重新渲染镜头。
- 从场次生成的新批次进入时间线时，自动选择对应场次，并沿用原场次成片与审片版本，避免重复生成第一场成片。
- 本场原始制作要求会传入分镜步骤；明确要求的连续穿门、后拉等一镜到底运动不再按室内外边界强制拆镜。
