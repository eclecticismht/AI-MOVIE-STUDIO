function storyActsContext(){
 ...
 const key=(script?.id||'draft')+'|'+(batch?.id||'unboarded'),shots=batch?StoryActs.ordered(D.shots.filter(s=>s.projectId===p.id&&s.storyboardBatchId===batch.id)):[];
 const saved=p.storyActs?.[key],draft=p.storyActs?.[(script?.id||'draft')+'|unboarded'];return {p,script,batch,key,shots,saved,acts:StoryActs.normalize(saved?.acts||draft?.acts,shots)};
}
function storyActsCommit(context,acts,backup=false){
 ...
 latest.storyActs={...(latest.storyActs||{}),[key]:{acts,history}};
 localStorage.setItem('aimovie_data',JSON.stringify(stored));p.storyActs=latest.storyActs;
}
function storyActsChange(action,id,value){try{
 ...
 if(action==='title'||action==='content')acts=acts.map(a=>a.id===id?{...a,[action]:value}:a);
 if(action==='auto'){if(!c.script?.content.trim())throw Error('请先填写剧本。');acts=StoryActs.auto(c.script.content,c.shots)}
 if(action==='restore'){if(!c.saved?.history?.length)throw Error('没有可恢复的场次分组');acts=StoryActs.normalize(c.saved.history.at(-1),c.shots)}
 storyActsCommit(c,acts,['auto','delete','restore'].includes(action));
 ...
 if(action!=='title'&&action!=='content')renderScripts();
 storyFlowNotice(c.p,action==='auto'?'已根据剧本标题划分场次；未找到标题时归为一场。请核对镜头归属，可用下拉框调整。':action==='delete'?'场次已删除，镜头保留并移入相邻场次；可恢复上次分组。':'场次内容已保存。');
...
storyFlowOutline=function(){
 ...
 return `<section class="story-outline story-acts"><div class="toolbar"><h2>场次内容</h2><button class="btn" onclick="storyActsChange('add')">＋ 添加场次</button><button class="btn gold" onclick="storyActsChange('auto')">根据剧本自动划分</button>${c.saved?.history?.length?'<button class="btn" onclick="storyActsChange(\'restore\')">恢复上次分组</button>':''}</div><p class="muted">场次 → 场景 → 镜头。现有镜头默认归入场次一；自动划分保留镜头和旧分组，不会重新生成视频。${c.batch&&c.batch.sourceContent!==c.script?.content?'当前镜头来自旧版剧本，请核对归属。':''}</p>${c.acts.map((a,index)=>{const shots=c.shots.filter(s=>a.shotIds.includes(s.id)),groups=StoryFlowModel.groups(shots,c.batch?.sourceContent||c.script?.content||'');return `<details class="card story-act" ${index===0?'open':''}><summary>${esc(a.title)} <span class="muted">${groups.length} 个场景 · ${shots.length} 个镜头</span></summary><div class="story-act-fields"><label>场次名称<input aria-label="场次 ${index+1} 名称" value="${esc(a.title)}" data-act="${esc(a.id)}" onchange="storyActsChange('title',this.dataset.act,this.value)"></label><label>场次内容<textarea aria-label="场次 ${index+1} 内容" data-act="${esc(a.id)}" placeholder="填写这一场发生的事情；不会覆盖剧本或镜头。" onchange="storyActsChange('content',this.dataset.act,this.value)">${esc(a.content)}</textarea></label><button class="btn" ${c.acts.length===1?'disabled':''} data-act="${esc(a.id)}" onclick="storyActsChange('delete',this.dataset.act)">删除场次（镜头移入相邻场次）</button></div>${groups.map(g=>`<details class="story-scene"><summary>场景 ${g.index} · ${esc(g.title)} · ${g.shots.length} 镜</summary>${g.shots.map((s,i)=>storyActShot(s,i,g,a,c.acts)).join('')}</details>`).join('')||'<p class="muted">暂无镜头。可把其他场次的镜头移入这里；尚未分镜时，可用当前剧本生成场景和镜头。</p>'}</details>`}).join('')}</section>`;
};
