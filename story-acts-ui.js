...
function storyActsChange(action,id,value){try{
 ...
 if(action==='restore'){if(!c.saved?.history?.length)throw Error('没有可恢复的场次分组');const prior=c.saved.history.at(-1),ids=new Set(prior.flatMap(a=>a.shotIds||[]));acts=StoryActs.normalize(prior,D.shots.filter(s=>s.projectId===c.p.id&&ids.has(s.id)))}
 ...
 storyFlowNotice(c.p,action==='auto'?'已根据剧本标题划分场次；未找到标题时归为一场。请核对镜头归属，可用下拉框调整。':action==='delete'?(acts.length?'场次已删除，镜头保留并移入相邻场次；可恢复上次分组。':'场次已删除。镜头、剧本和视频保留，可点击“恢复上次分组”撤销删除。'):'场次内容已保存。');
