...
function makeAss(shots){let start=0;const events=[];for(const s of shots.map(s=>s.renderMode==='screen'?{...s,dialogueEvents:[],subtitle:'',screenCards:s.screenSource==='text'&&s.screenCards?.some(c=>c.effect)?s.screenCards:[]}:s)){const duration=s.actualDuration??(17*Math.round((s.duration*24-5)/17)+5)/24,lines=subtitleLines(s.dialogueEvents?s.dialogueEvents.filter(e=>(e.type==='speech'&&s.subtitleTiming?.status!=='aligned')||(e.type==='screen'&&!(s.screenCards||[]).some(c=>(c.title+c.text).replace(/[\s，。！？、：:]/g,'').includes(e.text.replace(/[\s，。！？、：:]/g,''))))).map(e=>e.text).join('\n'):s.subtitle);lines.forEach((line,i)=>events.push(`Dialogue: 0,${assTime(start+i*duration/lines.length)},${assTime(start+(i+1)*duration/lines.length)},Default,,0,0,0,,${line}`));
