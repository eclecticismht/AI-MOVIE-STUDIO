...
http.createServer(async (request, response) => {
  ...
  if (request.method === "POST" && request.url === "/jobs") {
    try { const job = await body(request); if (!job.id || !job.prompt) return send(response, 400, {ok:false,error:"Job id and H3 prompt are required."}); const accepted={...job,connectorStatus:"已接收，等待 H3 Worker",receivedAt:new Date().toISOString()}; jobs.set(job.id, accepted); return send(response, 202, {ok:true,job:accepted}); }
