...
function comfyGraph(job) { const frames=h3FrameCount(job.duration), prompt=job.prompt.slice(0, 2000), prefix="AI_MOVIE_STUDIO/"+job.id; return {
  ...
  "5":{class_type:"MiniMaxH3Director",inputs:{model:["1",0],video_vae:["3",0],audio_vae:["4",0],clip:["2",0],task_type:"t2v — 文生视频(Text to Video)",global_prompt:prompt,cfg:1,seed:Math.floor(Math.random()*2147483647),frame_rate:24,width:864,height:480,ref_max_size:864,total_frames:frames,timeline_data:"",steps:25,sampler:"res_multistep",scheduler:"simple",shift_video:12,shift_audio:3,clear_vram_between_segments:true,export_source_images:false}},
