...
## 2026-09-22 — 分镜与原文同步、界面层级
- source-sync.js：精确、唯一的片段替换，明确区分本镜 sourceExcerpt、来源剧本和项目故事。一次持久化保存，故事保留上传前备份，项目记录最近 10 次同步；原批次 sourceContent 保留为历史快照。
- 时间线右侧、整批分镜与故事页镜头卡均可打开原文编辑；分镜编辑器提供“保存分镜并同步原文”，先保存再预览，起草仅使用行动和对白，不混入摄影提示词。
- 重复原句、失效引用、另一标签页的修改会阻止替换；共享引用的镜头标记需核对并阻止入队，本镜及相连承接镜头需重做，历史视频保留。
- UI：修改前后对照、明确同步范围、项目/场景/镜头路径、故事/资产/时间线流程导航、统一深色输入与焦点提示、响应式双栏。
- 验证：268 项测试通过；浏览器实际预览可定位第 1 镜引用并提示另 1 个镜头受影响，仅预览未写入用户故事；故事页 10 镜均有同步入口。前端更新无需重启服务。
