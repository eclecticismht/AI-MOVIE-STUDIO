...
function tlInspector(){
  ...
  if(TL.mode==='audio')body+=`<p class="muted">预览播放生成视频原声；下列声音设置在自动成片时应用。</p><label>本镜声音<select id="tl-audio-mode" onchange="TL.dirty=true"><option value="model">保留生成原声</option><option value="mute" ${s.audioMode==='mute'?'selected':''}>整镜静音</option><option value="voiceover" ${s.audioMode==='voiceover'?'selected':''}>独立画外声（电视 / 旁白 / 电话）</option><option value="replacement" ${s.audioMode==='replacement'?'selected':''}>替换为独立环境音</option></select></label><label>声音文件<input type="file" accept=".wav,.mp3" onchange="TL.dirty=true;importShotAmbience(this,'tl-audio-file')"></label><input id="tl-audio-file" type="hidden" value="${esc(s.audioAsset||'')}"><p id="tl-audio-file_status" class="muted">${s.audioAsset?'已有独立声音素材':'WAV / MP3，20MB 以内'}</p><button class="btn gold" onclick="tlSaveAudio()">保存本镜声音</button><button class="btn" onclick="TL.dirty=false;tlMessage('已撤销未保存的声音设置。');tlInspector()">撤销未保存设置</button><p class="muted">画内对白不能被静音或覆盖；电视、旁白和电话可使用独立画外声。保存后将标记本镜及承接镜头需重做，旧视频保留。</p><button class="btn" onclick="tlTools('audio')">项目声音任务</button>`;
  host.innerHTML=body;
}
