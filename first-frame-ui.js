...
function localFramePrompt(shot){return `Character positions and identities: ${Object.entries(shot.characterPositions||{}).filter(([id])=>(shot.characterIds||[]).includes(id)&&shot.assetStates?.[id]?.presence!=='offscreen').map(([id,position])=>(D.characters.find(c=>c.id===id)?.name||id)+': '+position+'; use only their own reference face, never the other person’s.').join(' / ')}\nScene and composition: ${shot.visual||shot.desc||''}\nCamera: ${shot.camera||''}\nOpening-frame state: ${shot.firstFrameIntent||'The people are at rest, before speaking or reaching out. Relaxed hands, no gestures. Do not invent an exchange, a payment, or an object being offered. Preserve only objects explicitly requested in the visual composition. The upcoming story actions will be generated separately as video.'}`}
...
function applyLocalFirstFrame(candidateUrl){
 ...
 try{const current={...shot,characterPositions:typeof readCharacterPositions==='function'?readCharacterPositions():shot.characterPositions,firstFrameIntent:document.getElementById('board_firstFrameIntent').value.trim(),visual:document.getElementById('board_visual').value.trim(),script:document.getElementById('board_script').value.trim(),camera:document.getElementById('board_camera').value.trim(),characterIds:selectedCharacterIds('shot_'+shot.id),sceneIds:selectedSceneIds('shot_'+shot.id),propIds:selectedPropIds('shot_'+shot.id)};if(typeof readAssetStateEditor==='function')current.assetStates=readAssetStateEditor(current);if(shot.localFrameSource!==localFrameSnapshot(current))throw Error('分镜或资产已经修改，请按最新内容重新生成首帧。');}catch(e){return alert(e.message)}
