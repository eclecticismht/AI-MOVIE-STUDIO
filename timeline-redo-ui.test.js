...
function setup(fail=false){
 ...
 const context=vm.createContext({D:structuredClone(data),TL:{},TimelineRedo:require('./timeline-redo'),FilmSourceSync:require('./film-source-sync'),structuredClone,AbortSignal,console,
  ...
  fetch:async(url,options)=>{if(!options)return {json:async()=>({runs:[]})};if(url==='/api/h3-prompts')return {ok:!fail,json:async()=>fail?{error:'AI failed'}:{prompts:[{id:'s',prompt}]}};submitted=JSON.parse(options.body);return {ok:true,json:async()=>({run:{id:'new'}})}}
 });
