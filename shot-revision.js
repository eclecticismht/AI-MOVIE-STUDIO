...
audioMode通常为model；用户明确要求完全静音时用mute并清空dialogue；未修改声音且原模式为replacement/overlay时可以保留该模式。assetStates返回本镜选中资产的状态对象，用已有id作键，值可含description、screenText、presence；删除屏幕文字时同步清除对应screenText，不生成图片URL。只对本镜改变服装或物品状态时在description明确覆盖原状态。breakContinuity为布尔值，要求与上一镜尾帧不兼容时为true。
