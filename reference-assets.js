...
function validateReferences(refs=[]){
  if(!Array.isArray(refs)||refs.filter(r=>r?.kind!=='videos').length>9||refs.filter(r=>r?.kind==='videos').length>3)throw Error('H3 每镜最多引用 9 张图片、3 个视频，请拆分镜头。');
  const ids=new Set();
  return refs.map(r=>{
    if(!r||typeof r.assetId!=='string'||ids.has(r.assetId)||!['characters','scenes','props','images','videos'].includes(r.kind)||typeof r.name!=='string'||!(r.kind==='videos'?/^ams-video-[a-f0-9]{64}\.mp4$/:/^ams-ref-[a-f0-9]{64}\.(png|jpg|webp)$/).test(r.file||''))throw Error('H3 资产参考图无效，请重新入队。');
    if(r.position!==undefined&&!['left','center','right','background'].includes(r.position))throw Error('人物站位无效');
    ids.add(r.assetId);return {assetId:r.assetId,kind:r.kind,name:r.name.slice(0,120),...(r.position?{position:r.position}:{}),notes:String(r.notes||'').slice(0,3000),file:r.file};
  });
}
...
function referencePrompt(prompt,refs){
  ...
  const mediaLabel=r=>r.kind==='videos'?`Video ${refs.filter(x=>x.kind==='videos').indexOf(r)+1}`:`Picture ${refs.filter(x=>x.kind!=='videos').indexOf(r)+1}`;
  ...
  return `subject_definitions:\n${refs.map((r,i)=>`<Subject ${i+1}> is ${r.name}, the ${r.kind==='characters'?'character':r.kind==='scenes'?'environment':r.kind==='videos'?'motion and visual reference':r.kind==='images'?'visual reference':'prop'} from <${mediaLabel(r)}>. ${referenceVisualNotes(r)} ${r.position?`Place this subject at the viewer’s ${r.position}, preserving only their own facial identity.`:''}`).join('\n')}\n\nsummary:\nCreate one continuous shot following the action below. Reference videos provide visual guidance only; do not copy their soundtrack, extra dialogue, subtitles or watermarks. Use referenced subjects only where the shot calls for them; off-screen speakers stay off-screen.\n\nretention_analysis:\n${refs.map((r,i)=>`<Subject ${i+1}>: fully_preserved - retain ${r.kind==='characters'?'facial identity, hair, age and clothing':r.kind==='scenes'?'spatial layout, architecture and furnishings':r.kind==='videos'?'the requested visual features and motion, never its soundtrack':r.kind==='images'?'the requested composition and visual features':'shape, material and color'} from <${mediaLabel(r)}>. Adapt camera angle and lighting to the shot. Explicit current-shot state notes override historical states or clothing in the asset profile; screen text reference cards specify content, not a new physical object. Do not copy unrelated people or backgrounds from a character or prop reference.`).join('\n')}\n\ndetailed_description:\n${detail}\n\noverall_soundscape:\n${part('overall_soundscape','non_diegetic_music','Natural location ambience; no added dialogue.')}\n\nnon_diegetic_music:\n${part('non_diegetic_music',null,'N/A')}`;
}
