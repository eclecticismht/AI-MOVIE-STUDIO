...
async function generateStoryAct(id){
 ...
 try{
  ...
  const script={id:v.scriptId,projectId:p.id,kind:'act-screenplay',title:a.title+' · 剧本',content:state.screenplay.content,sourceStory:story,storyUnderstanding:state.understanding,model,createdAt:new Date().toISOString(),...v.references};
  const batch={id:v.batchId,projectId:p.id,sourceScriptId:script.id,sourceTitle:script.title,sourceContent:script.content,sourceStory:story,storyUnderstanding:state.understanding,storyReview:state.review,model,createdAt:script.createdAt};
