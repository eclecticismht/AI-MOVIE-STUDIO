...
test('approval checks the latest disk screenplay even before a browser registers its new plan',async t=>{const data={projects:[{id:'p'}],shots:[{id:'a',projectId:'p',dur:4},{id:'b',projectId:'p',dur:4}],characters:[],scenes:[],props:[]},plan=input(),sync=require('./film-source-sync'),sources=[];for(const shot of data.shots){const fp=await sync.fingerprint(shot,data);plan.acts[0].shots.find(s=>s.shotId===shot.id).sourceFingerprint=fp;sources.push({...source(shot.id),shot:{...source(shot.id).shot,sourceFingerprint:fp}})}const service=createService({root:fixture(t),autoTick:false,sources:()=>sources,currentData:()=>data,assemble:async()=>({duration:8,shots:[],warnings:[]})});await service.sync(plan);await service.tick();const act=service.list('p')[0];data.shots[0].dialogue='更新后的原句';await assert.rejects(()=>service.approve(act.id,act.versions[0].id),/磁盘分镜已修改/);assert.equal(act.versions[0].approvedAt,undefined)});
