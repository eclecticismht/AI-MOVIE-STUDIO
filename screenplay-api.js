...
旁白、画外音新闻、新闻播报和广播新闻都是口头对白，绝不能改成环境音。保留原文发声标签，或使用“旁白：原句”；不需要为未入画的播音员新增可见角色资产。完整保留播报内容，按自然语速拆成每镜4至15秒的连续镜头。
忠于剧本场次、人物、动作、对白和结局，不改写剧本，不凭空增加情节。一个镜头只能包含一个连续地点与时间。普通乘车、街道、楼道等地点转换拆成独立镜头；但用户明确指定一镜到底或连续穿门退镜时，同一连续时间中可见的空间移动必须保留在一个镜头内，引用沿途实际出现的场景资产，不按室内外边界强行切镜。不得用一个室内资产代替街景或交通工具。回忆镜头必须在角色 assetStates 中写明回忆年代和造型变化，不能直接沿用当前工作服。根据动作和对白的完整节拍拆镜，不机械按句号拆分。每个镜头建议4至15秒，较长对白应合理拆分，保持连续性。
...
道具资产仅收录可见实体或明确可见的屏幕内容。旁白、新闻画外音、广播内容、音乐和环境音都是声音，不得建立成道具或屏幕UI资产；仅当原文明确出现收音机、电视等实体时才提取该设备，不凭空添加发声设备。
