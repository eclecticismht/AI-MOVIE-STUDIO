function h3JobSettings(){const p=activeProject();return {width:p.h3Width||864,height:p.h3Height||480,faceRefineMode:p.faceRefineMode||'auto'}}
...
function realProgressInfo(job){
  if(job.progress?.phase==='face-refine')return {label:'人脸检查 / 精修中',detail:job.faceRefine?.reason||job.status};const p=job.progress||{},size=`${job.width||864} × ${job.height||480}`;
  ...
  try{
    await Promise.allSettled(jobs.map(async job=>{
      try{
        ...
        job.faceRefine=remote.faceRefine;job.originalVideoUrl=remote.originalVideoUrl;job.progressUnavailable=false;job.progress=remote.progress;job.status=remote.connectorStatus;job.actualSeconds=remote.actualSeconds;
        if(remote.videoUrl){job.videoUrl=remote.videoUrl;job.completedAt=remote.completedAt;
          if(!D.generations.some(g=>g.jobId===job.id&&g.videoUrl===remote.videoUrl)){D.generations.push({id:uid('GEN'),projectId:job.projectId,jobId:job.id,sourceFingerprint:job.sourceFingerprint,shot:job.shot,version:job.version||'v001',status:'待审核',model:job.model,videoUrl:remote.videoUrl,createdAt:new Date().toLocaleString()});const shot=D.shots.find(s=>s.id===job.shot&&s.projectId===job.projectId);if(shot)shot.status='待审核'}
        }
