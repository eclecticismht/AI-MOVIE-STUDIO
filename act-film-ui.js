const AF={acts:[],key:'',preferredActId:null,registered:'',busy:false,selected:null,version:null,drafts:new Map(),error:'',approving:null,reviewError:'',reviewEpoch:0,confirmations:new Map()};
...
function renderActFilms(){
 ...
 host.querySelector('#actFilmWatched')?.addEventListener('change',e=>actFilmReviewDraft(version).watched=e.target.checked);
 host.querySelector('#actFilmReviewNote')?.addEventListener('input',e=>actFilmReviewDraft(version).note=e.target.value);
...
function actFilmReviewDraft(version){
 const key=version.id+'|'+version.review?.signature;
 if(!AF.confirmations.has(key))AF.confirmations.set(key,{signature:version.review?.signature,watched:false,note:''});
 return AF.confirmations.get(key);
}
function actFilmReviewControls(version){
 const r=version.review,draft=actFilmReviewDraft(version);
 if(!r)return '<p class="warn">等待载入声音、字幕与画面审片状态。</p>';
 return `<p>视频：已合成 · 声音：${r.speech==='checked'?'文字已核对':'待试听'} · 字幕：${r.subtitles==='aligned'?'已对齐':r.subtitles==='not_applicable'?'无口头字幕':'待核对'} · 画面：${r.visual==='reviewed'?'已人工核对':'待观看'}</p><label><input type="checkbox" id="actFilmWatched" ${draft.watched?'checked':''}>我已观看本版并核对人物、动作顺序、声音和字幕</label>${r.warnings.length?`<label>待核对项的实际观看结果<textarea id="actFilmReviewNote" maxlength="2000" rows="2" placeholder="填写听到或看到的实际结果；仍有错误请先修改。">${esc(draft.note)}</textarea></label>`:''}`;
}
...
async function actFilmApprove(act,version,host){
 ...
 try{
  ...
  const response=await fetch(`/api/act-films/${act.id}/approve`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({versionId:version.id,confirmation:actFilmReviewDraft(version)}),signal:AbortSignal.timeout(15000)}),out=await response.json();
