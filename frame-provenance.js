(function(root){
 ...
 function snapshot(shot,assets){return JSON.stringify({version:1,...(Object.keys(shot.characterPositions||{}).length?{characterPositions:shot.characterPositions}:{}),projectId:shot.projectId,shotId:shot.id,visual:shot.visual||shot.desc||'',script:shot.script||'',camera:shot.camera||'',intent:shot.firstFrameIntent||'',renderMode:shot.renderMode||'model',assets:assets.map(a=>({assetId:a.assetId,kind:a.kind,name:a.name,notes:a.notes||'',image:imageIdentity(a.imageUrl)}))});}
